...
const createPlugin: PluginFactory = (host, configDir) => {
            ...
            async handle(ctx) {
                ...
                if (/^[a-z0-9-]{1,64}$/.test(id)) {
                    for (const scraper of await scrapersPromise) {
                        if (packageOf.get(scraper) === id) setScraperEnabled(scraper.id, true);
                    }
        ...
        },
        {
            ...
            */
            ...
            async handle(ctx) {
                ...
                if (!/^https?:\/\//.test(target)) return { status: 400, body: "bad variant url" };
    ...
    async function extraStreamsFor(type: string, id: string, session?: unknown): Promise<{ from?: unknown; value: ExtraStream }[]> {
        const all = await scrapersPromise;
